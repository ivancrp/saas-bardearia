...
CREATE UNIQUE INDEX IF NOT EXISTS customers_company_user_unique ON public.customers(company_id, user_id) WHERE user_id IS NOT NULL;
...
BEGIN
  ...
  SELECT * INTO v_service FROM public.services
    WHERE id = p_service_id AND company_id = p_company_id AND active;
  IF NOT FOUND THEN RAISE EXCEPTION 'Serviço inválido'; END IF;

  SELECT * INTO v_pro FROM public.professionals
    WHERE id = p_professional_id AND company_id = p_company_id AND active;
  IF NOT FOUND THEN RAISE EXCEPTION 'Profissional inválido'; END IF;

  -- upsert customer for this company + user
...
REVOKE ALL ON FUNCTION public.book_public_appointment(uuid,uuid,uuid,timestamptz,text,text,text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.book_public_appointment(uuid,uuid,uuid,timestamptz,text,text,text) TO authenticated;
